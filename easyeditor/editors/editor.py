...
class BaseEditor:
        ...
        for i, request in enumerate(requests):
            ...
            if self.alg_name == 'IKE':
                ...
                if keep_original_weight is False:
                    self.model = deepcopy(edited_model)
                if 'locality' in all_metrics[i]['post'].keys():
                    ...
                    all_metrics[i]['pre']['locality'].pop(f'{locality_key}_output')
                    # all_metrics[i]['pre'].pop('locality')
        ...
        for record_chunks in self._chunks(requests, self.hparams.batch_size):
            ...
            for i, request in enumerate(record_chunks):
                ...
                if locality_inputs is not None:
                    for locality_key in request['locality'].keys():
                        assert len(all_metrics[tot]['post']['locality'][f'{locality_key}_output']) == \
                               len(all_metrics[tot]['pre']['locality'][f'{locality_key}_output'])
                        locality_result = []
                        for ans,label in zip(all_metrics[tot]['post']['locality'][f'{locality_key}_output'],all_metrics[tot]['pre']['locality'][f'{locality_key}_output']):
                            locality_result.append(np.mean(np.equal(ans, label)))
                        all_metrics[tot]['post']['locality'][f'{locality_key}_acc'] = locality_result
                        all_metrics[tot]['post']['locality'].pop(f'{locality_key}_output')
                    all_metrics[tot]['pre']['locality'].pop(f'{locality_key}_output')
                    # all_metrics[i]['pre'].pop('locality')
                tot+=1
    ...
    def edit_dataset(self,
        ...
        for record_chunks in tqdm(self._chunks(ds, num_edits), desc='Editing dataset', total=len(ds)/num_edits):
            ...
            for i, request in enumerate(record_chunks):
                all_metrics[tot]["pre"] = compute_edit_quality(self.model, self.model_name, self.hparams, self.tok, request,
                                                      self.hparams.device)                
                if 'locality' in all_metrics[tot]['post'].keys():
