...
def compute_edit_quality(
    ...
    if 'locality_prompt' in record.keys(): 
        ret['locality'].update(
            compute_locality_quality(model, model_name, hparams, tok, 'locality',
                                        record['locality_prompt'],
                                        record['locality_ground_truth'], device=device)
        )
    elif 'locality' in record.keys():
...
def compute_locality_quality(
    ...
    if 't5' in model_name.lower():
        ...
        loc_tokens, temp_acc = test_prediction_acc(model, tok, hparams, prompt, locality_ground_truth, device, locality=True)
    ...
    if 't5' in model_name.lower():
        ...
        ret = {
            f"{locality_key}_output": loc_tokens, 
            # f"{locality_key}_ground_truth_acc": temp_acc
        }
    return ret
