...
class MEND(EditableModel):
    ...
    def forward(self, *inputs, return_logits_only=True, **kwargs):
        if 'minigpt4' in self.config.model_name.lower() or 'blip' in self.config.model_name.lower():
            ...
            outputs = self.model(input_ids=kwargs['input_ids'], attention_mask=kwargs['attention_mask'], output_hidden_states=True,return_dict=True) 
            logits=_logits(outputs.logits)
            if return_logits_only:
                return logits      
            else:
                return logits, outputs.hidden_states[-1] 
            ...
            outputs = self.model(**kwargs, output_hidden_states=True,return_dict=True) 
            logits=_logits(outputs.logits)
            if return_logits_only:
                return logits      
            else:
                return logits, outputs.decoder_hidden_states[-1]
        else:
    ...
    def edit(self, batch, condition=None, detach_history=False, return_factors=False):
        if 'minigpt4' in self.config.model_name.lower() or 'blip' in self.config.model_name.lower():
            ...
            outputs = self.model(input_ids=batch['input_ids'], attention_mask=batch['attention_mask'], output_hidden_states=True,return_dict=True)
            # outputs = outputs[:, -batch['labels'].shape[-1]:, :]
            logits=_logits(outputs.logits)
            loss = self.edit_loss_fn(self.config, logits, batch["labels"])["nll"]
            
            # # calculate eigen
            # if 'eigen' in self.config.alg:
            #     gradients = torch.autograd.grad(loss,outputs.hidden_states[-1],retain_graph=True)
            #     fisher = gradients[0]
            ...
            #     # print("eigen: ", eigen)                    
            #     # loss = loss + eigen
            #     loss = self.config.C * loss + (1-self.config.C) * eigen
            #     # print("loss:",loss)
        elif 'llama' in self.config.model_name.lower():
