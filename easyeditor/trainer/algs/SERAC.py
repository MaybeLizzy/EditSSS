...
class SERAC(EditableModel):
    ...
    def forward(self, *inputs, return_logits_only=True, eps=torch.finfo(torch.float32).eps, pos_pairs=None, **kwargs):
        ...
        if self.config.freeze_cntr:
            ...
            outputs = self.replacement(**rep_cls_inputs, output_hidden_states=True,return_dict=True)
            rep_cls_logits = _logits(outputs.logits)
        ...
        if pos_pairs is not None:
            ...
            if self.config.freeze_cntr:
                ...
                outputs = self.replacement(**rep_gold_inputs, output_hidden_states=True,return_dict=True)
                rep_gold_logits = _logits(outputs.logits)
        else:
        ...
        if return_logits_only:
            ...
            return mixture_logits, cls_logits, rep_gold_logits, stats, outputs.hidden_states[-1]
