...
class BaseTrainer:
    def __init__(self, config, train_set: Dataset, val_set: Dataset):
        ...
        # self.model_dir = os.path.join(config.results_dir, config.alg, safe_model_name, f"C={self.config.C}")
        self.model_dir = config.results_dir
        if not (self.config.debug and not self.config.save) and not os.path.exists(self.model_dir):
            os.makedirs(self.model_dir)
        self.save_path = f"{self.model_dir}/C={self.config.C}"
    ...
    def run(self):
        ...
        if self.config.results_dir is not None:
            results_path = f"{self.model_dir}/C={self.config.C}_results.json"
        else:
            results_path = f"{os.getcwd()}/C={self.config.C}_results.json"
