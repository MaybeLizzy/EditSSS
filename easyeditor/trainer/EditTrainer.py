...
class EditTrainer(BaseTrainer):
    ...
    def edit_step(self, batch, training: bool):
        ...
        with torch.set_grad_enabled(training):                     
            ...
            if self.config.alg.startswith("SERAC"):
                post_edit_logits,_, _, _, hidden_states = edited_model(**batch["edit_inner"], return_logits_only=False)
                ...
                if 'SSS' in self.config.alg and training is True:
                    ...
                    for i in range(J.size(0)):
                        eigen+=J[i]**2                                  
                    l_edit = self.config.C * eigen + (1-self.config.C) * l_edit
            elif self.config.alg.startswith("MEND"):
                    post_edit_logits,hidden_states = edited_model(**batch["edit_inner"], return_logits_only=False)
                    l_edit = self.model.edit_loss_fn(
                        self.config, post_edit_logits, batch["edit_inner"]["labels"],
                    )["nll"]
            else:
        ...
        if self.config.alg.startswith("MEND"):
            ...
            if 'SSS' in self.config.alg and training is True:
                ...
                for i in range(J.size(0)):
                    eigen+=J[i]**2                
                l_total_edit = self.config.C * eigen + (1-self.config.C) * l_total_edit
        else:
            l_total_edit = self.config.cedit * l_edit + self.config.cloc * l_loc
