...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', default='data/portability/Subject_Replace/counterfact_subject_replace.json', type=str)
    parser.add_argument('--data', default='counterfact', type=str)
    parser.add_argument('--ds_size', default=None, type=int)
    parser.add_argument('--metrics_save_dir', default='data/portability/subject_replace', type=str)
    ...
    if args.data == "zsre":
        for edit_data_ in test_data:
            ...
            alter_subject_question.append(edit_data_['alter_subject_question'])        
            alternative_subject.append(edit_data_['alternative_subject'])
        ...
        for edit_data_ in test_data:
            ...
            alternative_sub = edit_data_["alternative_subject"]
            alter_subject_que = pp.replace("{}", alternative_sub)
            prompts.append(prompt)
            target_new.append(target_n)
            alter_subject_question.append(alter_subject_que)
            alternative_subject.append(alternative_sub)
            rephrase_prompts.append(edit_data_['paraphrase_prompts'])
    assert len(prompts) == len(alter_subject_question) == len(target_new) == len(alternative_subject)
    
    portability_inputs = {
        'subject_replace':{
            'prompt': alter_subject_question,
